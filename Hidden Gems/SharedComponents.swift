...
/// Full-screen, pinch-to-zoom image viewer presented when the user
/// taps (or pinches) a post photo. Black backdrop, dismiss button
/// in the top-right, standard iOS gestures:
///   • pinch to zoom (clamped to [1x, 5x])
///   • drag to pan while zoomed
///   • double-tap to toggle between 1x and 2.5x
///   • swipe down from 1x to dismiss
struct ZoomableImageViewer: View {
    ...
    private var panGesture: some Gesture {
        DragGesture()
            .onChanged { value in
                // Only pan while zoomed in — at 1x, DragGesture is
                // consumed by `dismissSwipeGesture` instead.
